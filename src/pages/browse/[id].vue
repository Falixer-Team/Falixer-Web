<template>
  <div
    v-if="pending"
    class="flex min-h-96 items-center justify-center rounded-3xl border border-neutral-700 bg-neutral-950"
  >
    <div class="flex flex-col items-center gap-3">
      <div
        class="border-brand-50 size-9 animate-spin rounded-full border-2 border-t-transparent"
      />
      <span class="text-default-font/55">正在加载扩展信息...</span>
    </div>
  </div>

  <div
    v-else-if="error || !extension"
    class="flex min-h-96 flex-col items-center justify-center rounded-3xl border border-neutral-700 bg-neutral-950 p-8 text-center"
  >
    <Icon name="memory:close" :size="36" class="text-default-font/35" />
    <h1 class="mt-4 text-2xl!">无法打开此项目</h1>
    <p class="text-default-font/55 mt-2">项目可能已下架，或当前网络连接异常。</p>
    <NuxtLink
      to="/browse"
      class="bg-brand-50 text-brand-900 mt-6 rounded-xl px-5 py-2.5 font-semibold"
    >
      返回扩展市场
    </NuxtLink>
  </div>
  ...
  <div v-else class="space-y-8">
    <div
      class="parent-focus overflow-hidden rounded-[2rem] border border-neutral-700 bg-neutral-950 shadow-sm transition-colors focus-within:border-neutral-500"
    >
      <div
      ...
      >
        ...
        >
          <div class="space-y-4 p-5 transition-colors lg:p-6">
            <div class="flex items-center gap-2">
              <span
                class="bg-brand-50/10 text-brand-50 rounded-full px-2.5 py-1 text-xs font-bold"
              >
                {{ extension.type === 'theme' ? '主题' : '扩展' }}
              </span>
              <span class="text-default-font/45 text-xs">
            ...
            <div class="flex justify-between gap-2">
              ...
              <NuxtLink
                ...
                tabindex="-1"
              >
                <ElementsButtonSmall> 编辑 </ElementsButtonSmall>
              </NuxtLink>
            </div>
          ...
          >
            <a
              ...
              :href="
                ...
                `utm_source=falixer&utm_medium=listing&utm_content=falixer_${extension.identifier}`
              "
              target="_blank"
        ...
        <template v-else>
          ...
          >
            <div class="md:max-w-100 p-4 text-left md:text-center">
              ...
              <h1 class="mb-2.5">暂无项目介绍</h1>
              <p>
                {{ extension.author.name }} 尚未填写详细介绍，请稍后再来看看。
              </p>
            </div>
          </div>
        </template>
      ...
      <div class="lg:min-w-100 lg:max-w-100">
        <div class="space-y-4 p-4">
              ...
              <div class="flex items-center gap-1 font-bold">
                ...
                <div>
                  ...
                  <span class="text-default-font/60 font-normal">
                    约 {{ extension.stats.panels }} 个 Falixer 面板
                  </span>
                </div>
              </div>
              <div
                ...
                <div class="flex items-center gap-2">
                  ...
                  <div>
                    <p v-if="moodmeter == 'love'">
                      用户对这个项目
                      <b class="font-extrabold text-purple-400">非常满意</b>
                    </p>
                    <p v-if="moodmeter == 'really like'">
                      用户对这个项目
                      <b class="font-extrabold text-green-400">十分认可</b>
                    </p>
                    <p v-if="moodmeter == 'like'">
                      用户对这个项目
                      <b class="text-brand-50 font-extrabold">整体满意</b>
                    </p>
                    <p v-if="moodmeter == 'meh'">
                      用户对这个项目的评价
                      <b class="font-extrabold text-yellow-600">较为一般</b>
                    </p>
                    <p v-if="moodmeter == 'dislike'">
                      用户对这个项目
                      <b class="font-extrabold text-red-400">不太满意</b>
                    </p>
                    <p class="text-default-font/60">
                      已收集 {{ totalReviews }} 条评价
                    </p>
                  </div>
                </div>
              </div>
          ...
          >
            ...
            >
              <span class="font-bold">关于开发者</span>
          ...
          >
            <div
              class="flex items-center justify-between gap-2 p-2 text-lg transition-colors"
            >
              <span class="font-bold">版本历史</span>
...
const { data: extension, pending, error } = await useAsyncData<Extension | null>(
...
const { data: description } = await useAsyncData(
  ...
  async (): Promise<MDCParserResult | null> => {
    ...
    try {
      ...
      console.warn('无法解析项目 Markdown：', error)
      return null
    }
  },
  {
